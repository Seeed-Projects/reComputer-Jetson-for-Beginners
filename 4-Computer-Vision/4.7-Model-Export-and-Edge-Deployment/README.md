...
> **Note:** Replace `yolo11s.pt` with your own trained model path from [Chapter 4.6](../4.6-Train-and-Deploy-Your-Own-Vision-Model/README.md), or use a pretrained model like `yolo11s.pt`.
...
```bash
yolo export model=yolo11s.pt format=engine imgsz=640 half=True device=0
```
...
> **Note:** This script requires `yolo11s.pt` and `yolo11s.engine` model files. Generate them first using the export commands above, or specify your own model paths with `--pt-model` and `--engine-model` arguments.
...
After exporting your model to TensorRT format (as shown above), use it for real-time inference:
...
![Custom model prediction](images/yolo-predict-custom.png)

> **Note:** This script requires `yolo11s.engine` file generated by the export commands above. If you don't have it yet, run the export first.
