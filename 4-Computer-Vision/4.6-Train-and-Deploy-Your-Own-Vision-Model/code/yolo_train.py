# YOLO11 模型训练示例
# 本脚本演示如何使用 Ultralytics YOLO11 的 Python API 训练自定义模型。
# 训练前请确保已准备好数据集并配置好 data.yaml 文件。
#
# 除了 Python API，也可以使用命令行（CLI）方式启动训练，两者功能等价：
#   yolo detect train data=<data.yaml路径> model=yolo11n.pt epochs=100 imgsz=640 device=0
#
...
def train_model(
    ...
    """使用 YOLO11 Python API 训练模型。

    Args:
        data_yaml: 数据集配置文件 data.yaml 的路径
        model_name: 预训练模型名称，如 'yolo11n.pt'（轻量级）
                    如需更高精度可替换为 'yolo11s.pt'、'yolo11m.pt' 等
        epochs: 训练轮次
        batch: 批量大小
        imgsz: 输入图像尺寸
...
if __name__ == "__main__":
    ...
    if not Path(data_yaml_path).exists():
        ...
        print("  yolo detect train data=<data.yaml路径> model=yolo11n.pt epochs=100 imgsz=640 device=0")
    else:
